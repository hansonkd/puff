...
impl PytestCommand {
    pub fn new<N: Into<Text>>(path: N) -> Self {
        let input_path: PathBuf = path
            .into()
            .parse()
            .expect("Could not convert text to path");
...
impl RunnableCommand for PytestCommand {
    ...
    fn runnable_from_args(&self, args: &ArgMatches, context: PuffContext) -> PuffResult<Runnable> {
        let (pytest_args, python_function) = Python::with_gil(|py| {
            ...
            pytest_args.append(format!("--rootdir={}", self.path.as_path().as_os_str().to_str().expect("Invalid path")))?;
        ...
        let fut = async move {
            ...
            let res = context.python_dispatcher().dispatch1(python_function, (pytest_args,))?;
            let r = res.await??;
