...
class Query:
    ...
    @classmethod
    def hello_world_object(cls, parents, context, /, my_input: List[SomeInputObject]) -> Tuple[List[SomeObject], List[Any]]:
        objs = [SomeObject(field1=0, field2="fa three"), SomeObject(field1=0, field2="fa so la de do"), SomeObject(field1=5, field2="None"), SomeObject(field1=1, field2="reda")]
        if my_input:
            for inp in my_input:
...
class Subscription:
    ...
    def hello_world(cls, parents, context, /, my_input: Optional[int] = None) -> List[str]:
